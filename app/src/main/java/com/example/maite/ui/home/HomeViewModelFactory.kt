...
class HomeViewModelFactory(private val context: Context) : ViewModelProvider.Factory {
    override fun <T : ViewModel> create(modelClass: Class<T>): T {
        if (modelClass.isAssignableFrom(HomeViewModel::class.java)) {
            ...
            val meetingApi = createMeetingApi()
            
            // 레포지토리 생성
            val proposalRepository = ProposalRepository(proposalApi)
            val meetingRepository = MeetingRepository(meetingApi)
            
            // ViewModel 생성 및 반환 (두 레포지토리 모두 전달)
            @Suppress("UNCHECKED_CAST")
            return HomeViewModel(proposalRepository, meetingRepository) as T
        }
    ...
    private fun createMeetingApi(): MeetingApi {
        // PreferencesUtil 초기화
        val preferencesUtil = PreferencesUtil(context)
        
        // 토큰 확인 로깅 추가
        val accessToken = preferencesUtil.getAccessToken()
        android.util.Log.d("HomeViewModelFactory", "createMeetingApi - Access Token: ${if (accessToken != null) "${accessToken.take(20)}..." else "null"}")
